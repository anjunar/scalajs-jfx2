...
object DemoI18n {
  ...
  val catalog: MessageCatalog =
    MessageCatalog(
      ...
      de(i18n"Virtual cards at scale", "Virtuelle Karten in großem Maßstab"),
      ...
      de(i18n"Virtual cards with a calm scroll story.", "Virtuelle Karten mit einer ruhigen Scroll-Geschichte."),
      de(i18n"Grid virtualization", "Grid-Virtualisierung"),
      de(i18n"Card collections should stay light even when they grow.", "Kartensammlungen sollen auch beim Wachsen leicht bleiben."),
      de(i18n"DataGrid is for predictable tile grids: media cards, article teasers, catalog entries. It derives the column count from a preferred card width, stretches the rendered cards to the viewport, and keeps the row height deterministic.", "DataGrid ist für vorhersehbare Kachel-Grids gedacht: Media-Cards, Artikel-Teaser, Katalogeinträge. Es leitet die Spaltenzahl aus einer bevorzugten Kartenbreite ab, streckt die gerenderten Karten auf den Viewport und hält die Zeilenhöhe deterministisch."),
      de(i18n"DataGrid is for fixed-size tiles: media cards, article teasers, catalog entries. It virtualizes rows and columns together, can prefetch remote windows, and can expose crawlable SSR pages through offset/limit.", "DataGrid ist für Kacheln mit fester Größe gedacht: Media-Cards, Artikel-Teaser, Katalogeinträge. Es virtualisiert Zeilen und Spalten gemeinsam, kann Remote-Fenster vorladen und crawlbare SSR-Seiten über offset/limit bereitstellen."),
      de(i18n"320x180", "320x180"),
      de(i18n"Preferred footprint with viewport-scaled width.", "Bevorzugtes Kartenmaß mit Viewport-skalierter Breite."),
      de(i18n"Card footprint with predictable layout.", "Kartenmaß mit vorhersehbarem Layout."),
      de(i18n"16px", "16px"),
      de(i18n"Gap keeps density readable.", "Der Abstand hält die Dichte lesbar."),
      de(i18n"Prefetch", "Prefetch"),
      de(i18n"Remote windows load before the user reaches them.", "Remote-Fenster laden, bevor der Nutzer sie erreicht."),
      de(i18n"Local DataGrid", "Lokales DataGrid"),
      de(i18n"Fixed-height cards with reactive local mutations and width that fills the viewport.", "Karten mit fester Höhe, reaktiven lokalen Mutationen und Breite, die den Viewport füllt."),
      de(i18n"Fixed-size cards with reactive local mutations in a stable viewport.", "Karten mit fester Größe und reaktiven lokalen Mutationen in einem stabilen Viewport."),
      de(i18n"Fixed-size cards with reactive local mutations and imperative scrollTo for focus jumps.", "Karten mit fester Größe, reaktiven lokalen Mutationen und imperativem scrollTo für Fokus-Sprünge."),
      de(i18n"Jump to item 12", "Zu Eintrag 12 springen"),
      de(i18n"Jump to last row", "Zur letzten Zeile springen"),
      de(i18n"Reset grid", "Grid zurücksetzen"),
      de(i18n"Total cards: ${I18n.named("count", 18)}", "Karten gesamt: {count}"),
      de(i18n"DataGrid DSL", "DataGrid DSL"),
      de(i18n"The control owns virtualization; the renderer only describes one card.", "Die Control besitzt die Virtualisierung; der Renderer beschreibt nur eine Karte."),
      de(i18n"Range-aware grid", "Bereichsbewusstes Grid"),
      de(i18n"RemoteListProperty can hydrate the visible window and keep SSR crawl pages honest.", "RemoteListProperty kann das sichtbare Fenster hydratisieren und SSR-Crawl-Seiten sauber halten."),
      de(i18n"Selection state", "Selection State"),
      de(i18n"Cards often need a calm selected state before a second action opens details.", "Karten brauchen oft erst einen ruhigen ausgewählten Zustand, bevor eine zweite Aktion Details öffnet."),
      de(i18n"No card selected yet.", "Noch keine Karte ausgewählt."),
      de(i18n"Selected card: ${I18n.named("title", "Atlas Memo 1")}", "Ausgewählte Karte: {title}"),
      de(i18n"Click and double click", "Click und Double Click"),
      de(i18n"Cards can react to both gestures, which is useful for selection on click and opening details on double click.", "Karten können auf beide Gesten reagieren, was fur Selektion per Click und das Offnen von Details per Double Click nützlich ist."),
      de(i18n"Click a card or double click it.", "Klicke auf eine Karte oder doppelklicke sie."),
      de(i18n"Click on ${I18n.named("title", "Atlas Memo 1")} at index ${I18n.named("index", 1)}", "Click auf {title} an Index {index}"),
      de(i18n"Double click on ${I18n.named("title", "Atlas Memo 1")} at index ${I18n.named("index", 1)}", "Double Click auf {title} an Index {index}"),
      de(i18n"Selected", "Ausgewählt"),
      de(i18n"Shared sizing", "Gemeinsames Sizing"),
      de(i18n"Same row math, flexible width", "Gleiche Zeilenlogik, flexible Breite"),
      de(i18n"Preferred width chooses the column count; rendered width stretches to fill the viewport.", "Die bevorzugte Breite wählt die Spaltenzahl; die gerenderte Breite streckt sich auf den Viewport."),
      de(i18n"Fixed tiles", "Feste Kacheln"),
      de(i18n"Same card box, many items", "Gleiche Kartenbox, viele Einträge"),
      de(i18n"Known width and height let the grid compute rows without measuring every cell.", "Bekannte Breite und Höhe lassen das Grid Zeilen berechnen, ohne jede Zelle zu messen."),
      de(i18n"Windowed loading", "Fensterweises Laden"),
      de(i18n"Prefetch stays close to view", "Prefetch bleibt nah am Viewport"),
      de(i18n"The grid can request nearby ranges before the user hits empty space.", "Das Grid kann benachbarte Bereiche anfordern, bevor der Nutzer auf leeren Raum trifft."),
      de(i18n"Crawlable SSR", "Crawlbares SSR"),
      de(i18n"Real pagination for crawlers", "Echte Pagination für Crawler"),
      de(i18n"offset and limit describe a repeatable HTML window while the client still upgrades to free scrolling.", "offset und limit beschreiben ein wiederholbares HTML-Fenster, während der Client trotzdem auf freies Scrollen hochrüstet."),
      de(i18n"Shared dimensions are the contract", "Gemeinsame Dimensionen sind der Vertrag"),
      de(i18n"DataGrid keeps height fixed and distributes the card width across the measured viewport.", "DataGrid hält die Höhe fest und verteilt die Kartenbreite über den gemessenen Viewport."),
      de(i18n"Fixed dimensions are the contract", "Feste Dimensionen sind der Vertrag"),
      de(i18n"DataGrid assumes each tile shares width and height so row math stays deterministic.", "DataGrid nimmt an, dass jede Kachel Breite und Höhe teilt, damit die Zeilenmathematik deterministisch bleibt."),
      de(i18n"Derived from column count, rendered tile width, fixed height, and gap to size the scroll surface.", "Wird aus Spaltenzahl, gerenderter Kachelbreite, fester Höhe und Abstand abgeleitet, um die Scroll-Fläche zu dimensionieren."),
      de(i18n"Derived from column count, tile size, and gap to size the scroll surface.", "Wird aus Spaltenzahl, Kachelgröße und Abstand abgeleitet, um die Scroll-Fläche zu dimensionieren."),
      de(i18n"Calculates start and end from scrollTop, viewportHeight, column count, and overscan rows.", "Berechnet Start und Ende aus scrollTop, viewportHeight, Spaltenzahl und Overscan-Zeilen."),
      de(i18n"Defines how far remote loading reaches beyond the visible cards.", "Definiert, wie weit Remote-Laden über die sichtbaren Karten hinausreicht."),
      de(i18n"In SSR the grid uses offset/limit and renders a real More link for the next window.", "Im SSR verwendet das Grid offset/limit und rendert einen echten More-Link für das nächste Fenster."),
      de(i18n"Remote loader", "Remote-Loader"),
      de(i18n"The loader only needs to answer index/limit requests with totalCount and nextQuery.", "Der Loader muss nur Index-/Limit-Anfragen mit totalCount und nextQuery beantworten."),
      de(i18n"Crawlable window", "Crawlbares Fenster"),
      de(i18n"The server renders a stable slice, while the browser restores scroll position from the same query.", "Der Server rendert einen stabilen Ausschnitt, während der Browser die Scroll-Position aus derselben Query wiederherstellt."),
