...
class TableView[S] extends ElementComponent[HTMLDivElement]("div") {

  private given ExecutionContext = ExecutionContext.global

  classProperty += "jfx-table-view"
  ...
  def scrollTo(index: Int): Unit = {
    if (disposed || !lifecycleInitialized) return
    val itemCount = getItems.totalLength
    if (itemCount == 0) return
    val clamped = math.max(0, math.min(itemCount - 1, index))
    nnDiv(scrollViewport).scrollTop = clamped * effectiveRowHeight
  ...
  private def initializeDefaultPlaceholder(): Unit = {
    nnDiv(defaultPlaceholder).className = "jfx-table-default-placeholder"
    nnDiv(defaultPlaceholder).textContent = "No content in table"
  }
  ...
  private def initializeObservers(): Unit = {
    ...
    nnDiv(scrollViewport).addEventListener("scroll", scrollListener)
    addDisposable(() => nnDiv(scrollViewport).removeEventListener("scroll", scrollListener))

    val resizeHandling = installResizeHandling()
    addDisposable(resizeHandling)
    addDisposable(selectionModel)
  ...
  private def refreshVisibleRows(
    ...
    val viewportHeight = math.max(nnDiv(scrollViewport).clientHeight.toDouble, 0.0)
    ...
    val firstVisibleIndex = math.floor(nnDiv(scrollViewport).scrollTop / rowHeight).toInt
  ...
  private def ensureRowPool(requiredRows: Int, columns: Seq[TableColumn[S, Any]]): Unit = {
    while (rowPool.length < requiredRows) {
      ...
      nnDiv(rowsContainer).appendChild(row.element)
  ...
  private def rebuildHeader(): Unit = {
    removeAllChildren(nnDiv(headerContent))
    ...
    currentColumns.zipWithIndex.foreach { case (column, index) =>
      ...
      nnDiv(headerContent).appendChild(cell)
    }
  }
  ...
  private def syncHeaderCellWidths(columns: Seq[TableColumn[S, Any]], columnWidths: Seq[Double]): Unit = {
    ...
    val headerCells = nnDiv(headerContent).children
  ...
  private def updateLayoutMetrics(
    ...
    val viewportWidth = math.max(nnDiv(scrollViewport).clientWidth.toDouble, 0.0)
    ...
    nnDiv(headerViewport).style.display = if (showHeader) "block" else "none"
    nnDiv(headerViewport).style.height = s"${headerHeight}px"
    nnDiv(headerContent).style.height = s"${headerHeight}px"
    nnDiv(headerContent).style.width = s"${contentWidth}px"
    nnDiv(headerContent).style.minWidth = s"${contentWidth}px"

    nnDiv(rowsContainer).style.width = s"${contentWidth}px"
    nnDiv(rowsContainer).style.minWidth = s"${contentWidth}px"
    nnDiv(rowsContainer).style.height = s"${contentHeight}px"
  ...
  private def refreshPlaceholder(): Unit = {
    ...
    val remote = currentRemoteItems
    nnDiv(placeholderLayer).style.display = if (showPlaceholder) "flex" else "none"
  ...
  private def mountPlaceholder(placeholder: NodeComponent[? <: Node]): Unit = {
    if (mountedPlaceholder == placeholder && placeholder.element.parentNode == nnDiv(placeholderLayer)) return

    detachMountedPlaceholder()
    removeAllChildren(nnDiv(placeholderLayer))

    nnDiv(placeholderLayer).appendChild(placeholder.element)
  ...
  private def showDefaultPlaceholder(): Unit = {
    if (nnDiv(defaultPlaceholder).parentNode != nnDiv(placeholderLayer)) {
      removeAllChildren(nnDiv(placeholderLayer))
      nnDiv(placeholderLayer).appendChild(nnDiv(defaultPlaceholder))
    }
  }

  private def hideDefaultPlaceholder(): Unit = {
    if (nnDiv(defaultPlaceholder).parentNode == nnDiv(placeholderLayer)) {
      nnDiv(placeholderLayer).removeChild(nnDiv(defaultPlaceholder))
    }
  }
  ...
  private def updateDefaultPlaceholderText(): Unit = {
    ...
    nnDiv(defaultPlaceholder).textContent = text
  }
  ...
  private def syncHeaderScroll(): Unit = {
    if (!lifecycleInitialized) return
    nnDiv(headerContent).style.transform = s"translateX(${-nnDiv(scrollViewport).scrollLeft}px)"
  }
  ...
  private def scheduleInitialRefresh(remainingFrames: Int): Unit = {
    ...
    window.requestAnimationFrame { _ =>
      if (!disposed) {
        refreshNow()
        if (!element.isConnected || nnDiv(scrollViewport).clientHeight == 0) {
  ...
  private def installResizeHandling(): Disposable = {
    ...
    if (!js.isUndefined(resizeObserverCtor) && resizeObserverCtor != null) {
      ...
      observer.observe(nnDiv(bodyWrapper))
  ...
  private def currentRenderedColumnWidths: Vector[Double] = {
    ...
    else resolveRenderedColumnWidths(columns, math.max(nnDiv(scrollViewport).clientWidth.toDouble, 0.0))
  }
