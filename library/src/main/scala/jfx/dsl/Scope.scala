...
object Scope {
  ...
  def singleton[T](provider: Scope ?=> T)(using scope: Scope, key: ServiceKey[T]): Unit =
    scope.singleton(provider)

  def scoped[T](provider: Scope ?=> T)(using scope: Scope, key: ServiceKey[T]): Unit =
    scope.scoped(provider)

  def transient[T](provider: Scope ?=> T)(using scope: Scope, key: ServiceKey[T]): Unit =
    scope.transient(provider)

  def inject[T](using scope: Scope, key: ServiceKey[T]): T =
    scope.inject[T]
